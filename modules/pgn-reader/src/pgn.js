...
const pgnReader = function (configuration) {
  ...
  function san(move) {
    ...
    if (that.configuration.notation === "short") {
      return (
        fig + disc + strike + notation.col + notation.row + prom + check + mate
      );
  ...
  function readMoves(moves) {
    ...
    function correctTurn() {
      ...
      if (
        getTurn(that.configuration.position) === "b" &&
        isMove(0) &&
        that.moves[0].turn === "w"
      ) {
        getMoves().forEach(function (move) {
  ...
  function findMove(moveRep) {
    if (!isNaN(moveRep)) {
      // the following goes only over the main line, move number cannot denote a variation
      moveRep = moveRep - 1;
      let move = getMove(0);
      while (moveRep > 0) {
        moveRep = moveRep - 1;
  ...
  function promoteMove(id) {
    ...
    const higherVariationMove =
      myFirst.prev == null
        ? getFirstMove()
        : getMove(getMove(myFirst.prev).next);
    let indexVariation;
  ...
  function startVariation(move) {
    return (
      ...
      (typeof move.prev != "number" ||
        getMoves()[move.prev].next !== move.index)
    );
  }
  ...
  function writePgn() {
        ...
        return (
          ...
          ((move.commentDiag.colorArrows &&
            ...
            (move.commentDiag.colorFields &&
              move.commentDiag.colorFields.length > 0))
        );
      };
      let arrows = (move) => {
        return move.commentDiag.colorArrows || [];
      };
      let fields = (move) => {
        return move.commentDiag.colorFields || [];
      };
    ...
    while (getMove(indexFirstMove) === null) {
      indexFirstMove += 1;
    }
  ...
  function addMove(move, moveNumber) {
    ...
    if (pgn_move.san.substring(0, 1) !== "O") {
      ...
      if (
        pgn_move.flags.includes(chess.FLAGS.CAPTURE) ||
        pgn_move.flags.includes(chess.FLAGS.EP_CAPTURE)
      ) {
  ...
  function getGameComment() {
    ...
    return that.games[that.currentGameIndex].gameComment
      ? that.games[that.currentGameIndex].gameComment
      : undefined;
  }

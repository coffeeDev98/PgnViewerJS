...
let pgnBase = function (boardId, configuration, lastStats = undefined) {
        ...
        function generateLink(link, nagDiv) {
          ...
          myLink.addEventListener("click", function () {
            function updateMoveSAN(moveIndex) {
              ...
              let _moveSpan = document.querySelector(
  ...
  function regenerateMoveSpan(_moveSpan, move) {
    ...
    if (move.notation && move.notation.fig) {
      ...
      const fig =
        !locale || figurine
          ? move.notation.fig
          : t("chess:" + move.notation.fig, locale);
  ...
  function generateMove(
    ...
    if (
      that.configuration.timeAnnotation != "none" &&
      move.commentDiag &&
  ...
  function unmarkMark(next) {
    ...
    removeClass(
      document.querySelector("#" + id("movesId") + " san.yellow"),
      "yellow"
    );
  ...
  function getShapes(commentDiag) {
    ...
    let arr = [];
    if (commentDiag && commentDiag !== undefined && commentDiag !== null) {
    ...
    function bindFunctions() {
      ...
      function togglePgn() {
        ...
        if (
          ...
          showPgn(str);
          document.querySelector("#" + boardId + " .textpgn").style.display =
            "block"; //slideDown(700, "linear")
        } else {
          document.querySelector("#" + boardId + " .textpgn").style.display =
            "none";
        }
  ...
  function resizeLayout() {
    ...
    if (document.getElementById(id("buttonsId"))) {
      document.getElementById(
        id("buttonsId")
      ).style.fontSize = `${_buttonFontSize}px`;
    }
    ...
    if (
      ...
      if (that.configuration.movesWidth) {
        ...
        _movesWidth = `${
          parseInt(that.configuration.width) - parseInt(_boardWidth)
        }px`;
      }
      if (that.configuration.layout === "left") {
